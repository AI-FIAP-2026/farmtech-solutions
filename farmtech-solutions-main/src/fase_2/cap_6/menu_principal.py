...
def carregar_config_json() -> dict:
    ...
    # Usar caminho relativo ao próprio arquivo para funcionar quando
    # o script for executado a partir da raiz do projeto
    nome_arquivo = os.path.join(os.path.dirname(__file__), "dados_cana.json")
    if not os.path.exists(nome_arquivo):
        print(f"{AMARELO}Arquivo dados_cana.json não encontrado. Usando padrão.{RESET}")
...
def salvar_no_banco(state: FarmData) -> None:
    """Salva os dados da cultura ativa em todas as tabelas do banco Oracle."""
    if not state.cultura_ativa:
        print(f"{VERMELHO}Nenhuma cultura ativa selecionada.{RESET}")
        return
    ...
    try:
        # Acessando os atributos do objeto cultura.insumos diretamente
        inserir_simulacao(
            id_simulacao=1,
            area_m2=cultura.area,
            metodo_colheita=cultura.financeiro.metodo_aplicacao or "Não informado",
            ...
            nome_cultura=cultura.cultura,
            status_qualidade=getattr(cultura, 'qualidade_texto', "Não analisado"),
            defeitos_graos=getattr(cultura, 'defeitos', 0),

            # CORREÇÃO AQUI: Acesso direto aos atributos do objeto de classe Insumos
            qnt_adubo=getattr(cultura.insumos, 'adubo', 0),
            qnt_agua=getattr(cultura.insumos, 'agua', 0),
            qnt_fosfato=getattr(cultura.insumos, 'fosfato', 0),
            nome_herbicida=getattr(cultura.insumos, 'herbicida', "N/A"),
            nome_pesticida=getattr(cultura.insumos, 'pesticida', "N/A"),
            nome_fertilizante=getattr(cultura.insumos, 'fertilizante', "N/A")
        )
    except Exception as e:
        print(f"{VERMELHO}Erro ao salvar no banco: {e}{RESET}")
    else:
        print(f"{VERDE}Simulação e dados relacionados salvos com sucesso em todas as tabelas!{RESET}")
...
def executar_estatisticas_r() -> None:
    ...
    # localizar script R relativo ao projeto
    script_path = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', '..', 'fase_1', 'cap_1', 'estatisticas_basicas.r'))
    if not os.path.exists(script_path):
        print(f"{VERMELHO}Script R não encontrado: {script_path}{RESET}")
        return

    try:
        resultado = subprocess.run(
            [rscript_path, script_path], capture_output=True, text=True, check=False)
        if resultado.returncode != 0:
            print(f"{VERMELHO}Erro ao executar R (code {resultado.returncode}):\nSTDERR:\n{resultado.stderr}\nSTDOUT:\n{resultado.stdout}{RESET}")
            return
        print(f"{VERDE}== Estatísticas (R) =={RESET}")
        print(resultado.stdout)
    except FileNotFoundError:
        print(f"{VERMELHO}Rscript não encontrado no caminho esperado: {rscript_path}{RESET}")
...
def consultar_previsao_tempo() -> None:
    ...
    # localizar script R relativo ao projeto
    script_path = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', '..', 'fase_1', 'cap_1', 'previsao_do_tempo.r'))
    if not os.path.exists(script_path):
        print(f"{VERMELHO}Script R não encontrado: {script_path}{RESET}")
        return
    ...
    try:
        resultado = subprocess.run(
            [rscript_path, script_path, cidade], capture_output=True, text=True, check=False)
        if resultado.returncode != 0:
            print(f"{VERMELHO}Erro ao executar R (code {resultado.returncode}):\nSTDERR:\n{resultado.stderr}\nSTDOUT:\n{resultado.stdout}{RESET}")
            return
